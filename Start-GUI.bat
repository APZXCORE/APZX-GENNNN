...
:: Check if pywebview is available for GUI
!PY_CMD! -c "import webview" 2>nul
if errorlevel 1 (
    echo.
    echo   [APZX] pywebview not found. Installing for GUI support...
    !PY_CMD! -m pip install pywebview
    if errorlevel 1 (
        echo.
        echo   [WARN] pywebview installation failed. Will open in browser.
    ) else (
...
:: Run the GUI
echo.
echo   [APZX] Starting APZX G3NNNN GUI...
echo.
